...
class Billingo_Admin
{
    ...
    public static function init_hooks()
    {
        ...
        add_action('woocommerce_admin_order_actions_end',[__CLASS__, 'add_listing_actions']);
    ...
    public static function billingo_admin_field_billingo_payment_settings_table($value)
    {
        ...
        <small>*: <?php esc_html_e('Csak ha fent be van kapcsolva a díjbekérő kiállítása', 'billingo'); ?>.</small>
        <?php
    }
    ...
    public static function billingo_admin_field_billingo_email_settings_table()
    {
        ...
        <table class="billingo_email_settings_table wc_input_table sortable widefat">
            ...
            <tbody>
                ...
                <tr>
                    <th class="th-border-bottom th-border-right"><?php esc_html_e('Gomb szöveg', 'billingo'); ?><br><small><?php esc_html_e('Ez lesz a gomb szövege az emailben,<br>ha a csatolás be van kapcsolva.', 'billingo'); ?></small></th>
                    <td>
                ...
                <tr>
                    <th class="th-border-bottom th-border-right"><?php esc_html_e('Gomb feletti szöveg', 'billingo'); ?><br><small><?php esc_html_e('Ez az üzenet jelenik meg az emailben,<br>ha a csatolás be van kapcsolva.', 'billingo'); ?></small></th>
                    <td>
        ...
        <table class="form-table">
            <tbody>
                ...
                <tr valign="top">
                    ...
                    <td class="forminp forminp-text"><a href="mailto:hello@billingo.hu">hello@billingo.hu</a></td>
                </tr>
                <tr valign="top">
                    <th scope="row" class="titledesc">
                        <label for="wc_billingo_support_code"><?php esc_html_e('Naplófájlok elérhetősége', 'billingo'); ?>:</label>
                    </th>
                    <td class="forminp forminp-text">/wp-content/uploads/billingo/</td>
                </tr>
            </tbody>
        </table>
        <?php
     ...
     *
     * @param string|array|WP_Screen $post_type The screen or screens on which to show the box 
     */
    public static function wc_billingo_add_metabox($post_type)
    {
        add_meta_box('custom_order_option', 'Billingo számla', [__CLASS__, 'render_meta_box_content'], 'shop_order', 'side');
    }
    ...
    public static function render_meta_box_content($post)
    {
        ...
        <?php } else { ?>
            ...
            <?php if (get_post_meta($post->ID, '_wc_billingo_own', true)): ?>
                <div style="text-align:center;" id="billingo_already_div">
                    ...
                    <p>
                        ...
                        <a id="wc_billingo_already_back" href="#" data-nonce="<?php echo(esc_attr(wp_create_nonce('wc_already_invoice'))); ?>" data-order="<?php echo(esc_attr($post->ID)); ?>"><?php esc_html_e('Visszakapcsolás', 'billingo'); ?></a>
                    </p>
                </div>
            <?php endif; ?>
            <?php
            $document_proforma = WC_Billingo::findOrderInvoice($post->ID, PWSBillingo::INVOICE_TYPE_PROFORMA);
            ...
            <?php if ($dijbekero_szamlaszam): ?>
                <p>
                    ...
                    <span class="alignright">
                        ...
                        <a href="<?php echo(esc_url(WC_Billingo::generateDownloadLink($post->ID, PWSBillingo::INVOICE_TYPE_PROFORMA))); ?>" target="_blank">
            ...
            <?php
            $document_storno = WC_Billingo::findOrderInvoice($post->ID, PWSBillingo::INVOICE_TYPE_STORNO);
            $szamlaszam = false;
            ...
            <?php if ($szamlaszam): ?>
                <div style="text-align:center;">
                    ...
                    <p>
                        <a href="<?php echo(esc_url(WC_Billingo::generateDownloadLink($post->ID, PWSBillingo::INVOICE_TYPE_STORNO))); ?>" id="wc_billingo_download" class="button button-primary" target="_blank">
                            <?php esc_html_e('Storno Számla megtekintése', 'billingo'); ?>
                        </a>
                    </p>
                </div>
            <?php elseif (WC_Billingo::isInvoiceGenerated($post->ID) && !get_post_meta($post->ID, '_wc_billingo_own', true)): ?>
                <div style="text-align:center;">
                    <p><?php esc_html_e('Számla sikeresen létrehozva.', 'billingo'); ?></p>
                    <p>
                        <?php esc_html_e('A számla sorszáma:', 'billingo'); ?>
                        <strong><?php echo(esc_html(WC_Billingo::getInvoiceNumberGenerated($post->ID))); ?></strong>
                    </p>
                    <p>
                        <a href="<?php echo(esc_url(WC_Billingo::generateDownloadLink($post->ID, PWSBillingo::INVOICE_TYPE_INVOICE))); ?>" id="wc_billingo_download" class="button button-primary" target="_blank">
                            <?php esc_html_e('Számla megtekintése', 'billingo'); ?>
                        </a>
                    </p>
                    <?php if (get_post_meta($post->ID, '_wc_billingo_storno', true) != 1) { ?>
                        <p>
                            <a href="#" id="wc_billingo_storno" data-order="<?php echo(esc_attr($post->ID)); ?>" data-nonce="<?php echo(esc_attr(wp_create_nonce('wc_storno_invoice'))); ?>" class="button button-primary"><?php esc_html_e('Sztornózás', 'billingo'); ?></a>
                        </p>
                    <?php } ?>
                </div>
            <?php else: ?>
                <div style="text-align:center;<?php if (get_post_meta($post->ID, '_wc_billingo_own', true)): ?>display:none;<?php endif; ?>" id="wc-billingo-generate-button">
                    <p>
                        <a href="#" id="wc_billingo_generate" data-order="<?php echo(esc_attr($post->ID)); ?>" data-nonce="<?php echo(esc_attr(wp_create_nonce('wc_generate_invoice'))); ?>" class="button button-primary"><?php esc_html_e('Számlakészítés', 'billingo'); ?></a>
                        <br>
                    ...
                    <div id="wc_billingo_options_form" style="display:none;">
                        <div class="fields">
                            ...
                            <select name="wc_billingo_invoice_type" id="wc_billingo_invoice_type">
                                <option value="invoice"><?php esc_html_e('Számla', 'billingo'); ?></option>
                                <option value="proforma"><?php esc_html_e('Díjbekérő', 'billingo'); ?></option>
                                <option value="draft"><?php esc_html_e('Piszkozat', 'billingo'); ?></option>
                            </select>
                        </div>
                        <a id="wc_billingo_already" href="#" data-nonce="<?php echo(esc_attr(wp_create_nonce('wc_already_invoice'))); ?>" data-order="<?php echo(esc_attr($post->ID)); ?>">
