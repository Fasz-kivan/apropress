...
class Membership {
	...
	private function set_membership_data() {
		...
		if ( null !== $membership ) {
			...
			$this->picture_limit    = $membership->picture_limit;
	...
	public function get_picture_limit() {
		return $this->picture_limit;
	}
	...
	public function update_membership( $payment ) {
		if ( $this->id ) {
			...
			if ( apply_filters( 'rtcl_store_membership_carry_forward', true ) && $current_date < $expiry_date ) {
			    $c_posted_ads= $this->get_posted_ads();
			    $c_remaining_ads=$this->get_remaining_ads();
			    if($new_ads<=$c_posted_ads+$c_remaining_ads)
			    {
				...
				if($c_posted_ads > $new_ads)
				{
				    $counter=$c_posted_ads - $new_ads;
				    ...
				    $data['posted_ads']=$new_ads;
				    $data['ads']= 0;
				}
				else
				{
				$data['ads']         = $new_ads - $c_posted_ads;
				}
				if($new_ads < 10) {
					$data['picture_limit']=5;
				}
				else{
					$data['picture_limit']=10;
				}
				$expired             = false;
			} else {
				$current_date->add( new \DateInterval( "P{$days}D" ) );
