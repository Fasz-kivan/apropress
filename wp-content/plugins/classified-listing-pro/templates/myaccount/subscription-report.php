...
if ( ! empty( $subscriptions ) ) {
	...
	foreach ( $subscriptions as $subscription ) {
		...
		<div class="rtcl-sub-item" data-gateway="<?php echo esc_attr( $gateway->id ) ?>"
			...
			<?php if ( ! empty( $metaData->cc ) ): ?>
				<div class="rtcl-sub-info cc-info">

					<div class="rtcl-subi-cc">
						<div class="cc-type"
							 data-id="<?php echo esc_attr( $metaData->cc->type ) ?>"><?php echo esc_html( $metaData->cc->type ) ?></div>
						<div class="cc-number">XXXX-<?php echo esc_html( $metaData->cc->last4 ) ?></div>
						<div class="cc-expiry">(<?php echo esc_html( $metaData->cc->expiry ) ?>)</div>
