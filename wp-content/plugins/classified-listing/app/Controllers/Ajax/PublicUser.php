...
class PublicUser
{
	...
	public static function rtcl_registration_request_handler() {
		...
		if ($newsletter) {
            ...
            $wpdb->insert($table_name, array('email' => $user_email));
        }
	...
	public function rtcl_delete_listing() {
		...
		if (Functions::verify_nonce()) {
			...
			if ($post_id && Functions::current_user_can('delete_' . rtcl()->post_type, $post_id)) {
			    ...
			    global $wpdb;
			    $is_expired=get_post_status($post_id);
		        $author_id = get_post_field( 'post_author', $post_id );
		        $table = $wpdb->prefix . "rtcl_membership";
		        $query = $wpdb->get_row($wpdb->prepare( "SELECT * FROM {$table} WHERE user_id = %d", $author_id));
		        $updated_posted_ads=$query->posted_ads;
		        $updated_ads=$query->ads;
		        $updated_posted_ads=$updated_posted_ads-1;
		        if($is_expired != 'rtcl-expired') {
		        $updated_ads=$updated_ads+1;
		        }
		        if($updated_posted_ads<0){
		            $updated_posted_ads=0;
		        }
		        $data = array(
			        'posted_ads'      => $updated_posted_ads,
			        'ads'          => $updated_ads
		        );
		        $where  = [
			        'user_id' => $author_id,
